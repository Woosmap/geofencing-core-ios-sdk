...
final class CLMonitorBackendTests: XCTestCase {
    ...
    // MARK: - Reconciling the mirror against CLMonitor

    /// CoreLocation evicts conditions on its own — `conditionLimitExceeded` shows
    /// up in field logs for regions the SDK never asked to remove. Below iOS 17.2
    /// nothing reports that at all. The mirror has to be able to catch up.
    func test_prune_dropsWhatCLMonitorNoLongerHolds() throws {
        guard #available(iOS 17.0, *) else { throw XCTSkip("CLMonitor requires iOS 17") }
        let name = uniqueName("Prune")
        let backend = CLMonitorBackend(monitorName: name)
        backend.start(identifier: "poi<id>kept<id>", center: anchor, radius: 100)
        backend.start(identifier: "poi<id>evicted<id>", center: anchor, radius: 100)

        let dropped = backend.prune(keeping: ["poi<id>kept<id>"],
                                    in: MonitorSession.shared(named: name))

        XCTAssertEqual(dropped, ["poi<id>evicted<id>"])
        XCTAssertEqual(backend.monitoredCircularIdentifiers, ["poi<id>kept<id>"])
    }

    /// Pruning must not re-register. A condition evicted for exceeding the limit
    /// would be re-added, evicted, and re-added for as long as the app runs;
    /// recovery belongs to the SDK's own refresh, under its own budget.
    func test_prune_doesNotReRegister() throws {
        guard #available(iOS 17.0, *) else { throw XCTSkip("CLMonitor requires iOS 17") }
        let name = uniqueName("NoReAdd")
        let backend = CLMonitorBackend(monitorName: name)
        backend.start(identifier: "poi<id>evicted<id>", center: anchor, radius: 100)

        backend.prune(keeping: [], in: MonitorSession.shared(named: name))

        XCTAssertTrue(backend.monitoredGeofences.isEmpty,
                      "the mirror stops claiming it; re-adding is the refresh's job")
    }

    /// Nothing evicted means nothing to say, and no log.
    func test_prune_isANoOpWhenTheMirrorAgrees() throws {
        guard #available(iOS 17.0, *) else { throw XCTSkip("CLMonitor requires iOS 17") }
        let name = uniqueName("Agree")
        let backend = CLMonitorBackend(monitorName: name)
        backend.start(identifier: "poi<id>a<id>", center: anchor, radius: 100)

        let dropped = backend.prune(keeping: ["poi<id>a<id>"],
                                    in: MonitorSession.shared(named: name))

        XCTAssertTrue(dropped.isEmpty)
        XCTAssertEqual(backend.monitoredCircularIdentifiers, ["poi<id>a<id>"])
    }

    /// A pruned identifier's last known state goes with it, so when the refresh
    /// re-adds the region the "already inside" event reads as an initial
    /// determination rather than a crossing the user never made.
    func test_prune_forgetsTheLastKnownState() throws {
        guard #available(iOS 17.0, *) else { throw XCTSkip("CLMonitor requires iOS 17") }
        let name = uniqueName("Forget")
        let session = MonitorSession.shared(named: name)
        _ = session.decide(identifier: "poi<id>a<id>", state: .unsatisfied, eventDate: Date())

        _ = session.prune(keeping: [])

        XCTAssertEqual(session.decide(identifier: "poi<id>a<id>", state: .satisfied,
                                      eventDate: Date().addingTimeInterval(1)),
                       .transition(didEnter: true, isInitial: true))
    }
}
